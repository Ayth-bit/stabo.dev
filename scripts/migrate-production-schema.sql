-- Production Schema Migration: Integrate existing threads with board system
-- This migration brings production schema in line with local development schema
...
-- Remove columns that shouldn't exist in posts (if they exist)
DO $$ 
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'posts' AND column_name = 'link') THEN
        ALTER TABLE posts DROP COLUMN link;
    END IF;
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'posts' AND column_name = 'color') THEN
...
BEGIN
    ...
    LOOP
        -- Calculate distance using simplified formula (meters)
        distance := SQRT(
...
-- Update the trigger function for updated_at (if not exists)
