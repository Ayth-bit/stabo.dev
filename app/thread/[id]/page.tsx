...
const MAX_POSTS = Number.parseInt(process.env.NEXT_PUBLIC_MAX_POST_COUNT || '1000', 10);
        ...
        navigator.geolocation.getCurrentPosition(
          async (position) => {
            ...
            if (distance > READ_RADIUS_KM) {
              setPageStatus({
                loading: false,
                error: `このスレッドは範囲外です (距離: ${distance.toFixed(2)}km)。`,
                isAccessAllowed: false,
                canWrite: false,
              });
            } else {
              ...
              const errorMsg = canWrite
                ? null
                : `読み取り専用モード (書き込みするには${WRITE_RADIUS_KM * 1000}m以内に近づいてください)`;
    ...
    return (
      <div style={{ textAlign: 'center', padding: '50px' }}>
        <p style={{ color: 'red', fontWeight: 'bold' }}>
          {pageStatus.error || 'アクセスできませんでした。'}
        </p>
        <button type="button" onClick={() => router.push('/')} style={{ marginTop: '20px' }}>
          トップページに戻る
        </button>
      </div>
    );
  ...
  return (
    ...
    >
      ...
      <p style={{ textAlign: 'center', fontSize: '0.9em', color: 'var(--text-tertiary)' }}>
        投稿数: {thread.post_count} / {MAX_POSTS}
      </p>
        ...
        <div style={{ maxHeight: '400px', overflowY: 'auto', padding: '10px' }}>
          {posts.map((post) => (
            ...
            >
              ...
              >
                <strong style={{ color: post.color || 'var(--text-primary)' }}>
                  {post.author_name || '匿名'}
                </strong>
              ...
              <p
                style={{
                  fontFamily: post.font_family || 'inherit',
                  color: post.color || 'var(--text-primary)',
                  whiteSpace: 'pre-wrap',
      ...
      >
        ...
        {pageStatus.error && (
          <p style={{ color: pageStatus.canWrite ? 'red' : 'orange', textAlign: 'center' }}>
            {pageStatus.error}
          </p>
        )}
        <form
          ...
          style={{
            ...
            gap: '15px',
            opacity: pageStatus.canWrite ? 1 : 0.6,
            pointerEvents: pageStatus.canWrite ? 'auto' : 'none',
          }}
        >
          <input
            type="text"
