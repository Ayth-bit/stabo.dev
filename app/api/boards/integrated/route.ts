...
export async function GET(request: NextRequest) {
  try {
    ...
    if (filter === 'accessible') {
      ...
      // 駅は閲覧可能範囲で最大3件
      filteredBoards = boardsWithDistance.filter(board => 
        board.type === 'station' && board.isViewable
      ).slice(0, 3);
    } else if (filter === 'ward') {
      // 区は閲覧可能範囲で最大3件
      filteredBoards = boardsWithDistance.filter(board => 
        board.type === 'ward' && board.isViewable
      ).slice(0, 3);
    } else if (filter === 'park') {
      // 公園は閲覧可能範囲で最大3件
      filteredBoards = boardsWithDistance.filter(board => 
        board.type === 'park' && board.isViewable
      ).slice(0, 3);
    } else {
      // 全て（閲覧可能範囲で最大3件）
    ...
    return NextResponse.json({
      ...
      allStats: {
        station: Math.min(3, boardsWithDistance.filter(b => b.type === 'station' && b.isViewable).length),
        ward: Math.min(3, boardsWithDistance.filter(b => b.type === 'ward' && b.isViewable).length),
        park: Math.min(3, boardsWithDistance.filter(b => b.type === 'park' && b.isViewable).length),
