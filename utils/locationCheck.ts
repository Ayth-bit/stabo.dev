  ...
  return new Promise((resolve) => {
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        wardDistances.forEach((w) => {
          const status = w.inRange ? "OK" : "NG";
          console.log(
            `  ${w.ward}: ${w.distance.toFixed(3)}km (範囲: ${
              w.radius
            }km) [${status}]`
          );
        });
        ...
        console.log(
          `[LOCATION] 最寄り区: ${nearestWard} (${shortestDistance.toFixed(
            3
          )}km)`
        );
      ...
      (error) => {
        ...
        if (error.code === 1) {
          ...
          errorMessage =
            "位置情報を取得できませんでした。ネットワーク接続またはGPS設定を確認してください。";
